...
format — that fights the model's native reasoning. **V3** is a standard chat model where
an explicit chain-of-thought request is fine. Both are strongly bilingual (中/英).
...
1. **R1 (reasoning)**: give a **complete, unambiguous problem statement** and the answer
   ...
   forced reasoning template. Put any constraints plainly.
