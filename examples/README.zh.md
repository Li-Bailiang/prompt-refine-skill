...
## 9. 宿主模型：Z.ai GLM — 长任务 coding agent 边界

**重构前**
> 帮我给这些供应商页面做个爬虫和看板

**重构后**（`strategies/zai-glm.md`）
> 目标：基于下方 URL 构建供应商页面爬虫和一个小型看板。
>
> GLM 使用场景：coding agent，启用 thinking mode，并允许使用真实工具。
>
> 完成标准：
> 1. 爬虫提取供应商名称、产品类别、价格信号、联系 URL
> 2. 看板按类别分组，并标记缺失字段
> 3. 测试覆盖一个正常页面、一个缺失价格页面、一个无法访问页面
>
> 工具边界：只使用给定 URL 和本地仓库文件。不要编造供应商数据；缺失值写为 `null`。
>
> 实现后停止，并报告：修改文件、测试命令、剩余风险。
>
> ```text
> [供应商 URL]
> ```
