...
本 skill 也已发布到 npm:[`prompt-refine-skill`](https://www.npmjs.com/package/prompt-refine-skill)（带版本号的发布）。npm 不会自动注册 Agent Skill；你可以把它当作版本化来源，解包到工具的 skills 目录中：
...
上面的 `git clone` 和 `degit` 命令会直接把文件放到工具的 skills 目录中。
...
| 未知宿主模型 | `strategies/universal.md` | 保守兜底策略 |
...
Prompt Refine 还包含一个小型非退化检查集，覆盖清晰或强约束提示词：JSON/config
输出、字数限制、语言保持和直接回答任务。当前 6 条 guard prompts 上，refine 在
12 次交换位置裁判中胜率为 **66.7%**（8 胜 / 4 负 / 0 平）。这应被视为早期质量
护栏，而不是对所有清晰提示词的广泛证明。
...
评测 harness、prompts、rubrics、匿名 answer pairs、judge JSON、运行命令和已提交的
结果汇总都在 GitHub 仓库的 [`eval/`](eval/) 目录中。eval 文件不会进入 npm 包，
以保持普通 skill 安装足够轻量。
...
- **尽力而为，并非确定性。** 它在激活指令仍留在模型上下文时生效；对话过长被压缩后可能失效，
  需要重新 `/prompt-refine`。
- **依赖宿主模型遵循元指令。** 对不能稳定执行“先静默重构、再回答”的模型，收益较小。
...
如果 Prompt Refine 帮你省了时间，欢迎给仓库点一个 ⭐ —— 这能实实在在地帮助更多人发现这个项目。
